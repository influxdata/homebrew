...
class Telegraf < Formula
  ...
  url "https://github.com/influxdb/telegraf.git", :using => :git, :tag => "v0.1.0"

  depends_on "go" => :build

  go_resource "github.com/influxdb/influxdb" do
    url "https://github.com/influxdb/influxdb.git",
        :revision => "729bdec7dd3f98f0ab4f0079c21c64d197cf87be"
  end
  ...
  def install
    ...
    cd telegraf_path do
      system "go", "build", "-ldflags", "-X main.Version 0.1.0", "cmd/telegraf/telegraf.go"
    end

    bin.install telegraf_path/"telegraf" => "telegraf"
